...
namespace TriLib
{
    ...
    public class AssetLoader : IDisposable
    {
        ...
        /// <summary>
        /// Base <see cref="UnityEngine.Material"/> used to load materials.
        /// </summary>
        ...
        /// @warning To ensure your materials will be loaded, don´t remove the __StandardMaterial.mat__ and __StandardSpecularMaterial.mat__ files included in the package.
        /// </summary>
        private bool LoadStandardMaterials()
        {
            ...
            return _standardBaseMaterial != null && _standardSpecularMaterial != null;
        }
        ...
        /// @warning To ensure your materials will be loaded, don´t remove the __StandardMaterial.mat__ and __StandardSpecularMaterial.mat__ files included in the package.
        /// </summary>
        /// <param name="filename">Filename uset to load the <see cref="UnityEngine.GameObject"/>.</param>
        ...
        private IntPtr ImportFile(string filename, AssetLoaderOptions options)
        {
            ...
            {
                scene = AssimpInterop.ai_ImportFile(filename, (uint)options.PostProcessSteps);
            }
            return scene;
        }
                ...
                else
                {
                    ...
                    if (numDiffuse > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            ...
                            Texture2DUtils.LoadTextureFromFile(scene, path, textureName, unityMaterial, "_MainTex", wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression, filenameWithoutExtension);
                    ...
                    if (numEmissive > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            var wrapMode = mapMode == (uint)TextureWrapMode.Clamp
                                ...
                                : TextureWrapMode.Repeat;
                            var textureName = StringUtils.GenerateUniqueName(path);
                            Texture2DUtils.LoadTextureFromFile(scene, path, textureName, unityMaterial, "_EmissionMap", wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression, filenameWithoutExtension);
                    ...
                    if (numSpecular > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            var wrapMode = mapMode == (uint)TextureWrapMode.Clamp
                                ...
                                : TextureWrapMode.Repeat;
                            var textureName = StringUtils.GenerateUniqueName(path);
                            Texture2DUtils.LoadTextureFromFile(scene, path, textureName, unityMaterial, "_SpecGlossMap", wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression, filenameWithoutExtension);
                    ...
                    if (numNormals > 0)
                    {
                                ...
                                out blendMode, out op, out mapMode))
                        {
                            var wrapMode = mapMode == (uint)TextureWrapMode.Clamp
                                ...
                                : TextureWrapMode.Repeat;
                            var textureName = StringUtils.GenerateUniqueName(path);
                            Texture2DUtils.LoadTextureFromFile(scene, path, textureName, unityMaterial, "_BumpMap", wrapMode, texturesPath, OnTextureLoaded, options.TextureCompression, filenameWithoutExtension);
        ...
        {
            return _nodeDataDictionary.ContainsKey(name) || string.IsNullOrEmpty(name) ? StringUtils.GenerateUniqueName(id) : name;
        }
